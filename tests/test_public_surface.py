...
class PublicSurfaceContractTests(unittest.TestCase):
    ...
    def test_country_config_matches_public_surface(self):
        ...
        self.assertEqual(config.display_name, "Netherlands")
        self.assertEqual(config.adjective, "Dutch")
        self.assertEqual(len(config.supported_factors), 8)
    ...
    def test_public_wrapper_defaults(self):
        ...
        self.assertIn('WPD_APP_TITLE", "Dutch Politics Data"', wrapper)
        self.assertIn('WPD_EXPOSE_COUNTRIES", "netherlands"', wrapper)
        self.assertIn('WPD_PROFILE", "netherlands_only"', wrapper)
